...
from backend.app.models.response_generation import (
    generate_response_ollama,
    generate_doc_with_openai,
)
from backend.app.prompts.builder import build_prompt
from backend.app.schemas.outlines.protocol_outlines import PROTOCOL_OUTLINE
from backend.app.schemas.protocol_template import ProtocolInput
...
async def download_protocol_document():
    try:
        ...
        return FileResponse(
            pdf_path, media_type='application/pdf', filename="protocol_document.pdf"
        )

    except Exception as e:
        raise HTTPException(
            status_code=500, detail="Error downloading the protocol document."
        )
